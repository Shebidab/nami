...
// The `shortcuts` icon is a drawing of the Command key — the looped-square
// glyph itself. On a keyboard that has no such key it is a picture of nothing,
// so off macOS the shortcuts surfaces get the keyboard instead. Handled here
// rather than at the three call sites, because it is a fact about the icon.
export function helpIcon(name) {
  const paths = HELP_ICONS[name === 'shortcuts' && !MAC ? 'keyboard' : name];
  if (!paths) return '';
