...
test('masterPath: project scope in the project, user scope under ~/.nami', () => {
  assert.equal(masterPath({ scope: 'project', projectPath: PROJ, homeDir: HOME }), abs('proj', 'connections.json'));
  assert.equal(masterPath({ scope: 'user', projectPath: PROJ, homeDir: HOME }), abs('home', 'u', '.nami', 'connections.json'));
});
...
test('upsertMaster writes the standard mcpServers shape and round-trips', () => {
  ...
  const doc = JSON.parse(io.files[abs('proj', 'connections.json')]);
...
test('upsertMaster in a git repo adds connections.json to .gitignore once', () => {
  const io = memIo({ [abs('proj', '.git')]: '', [abs('proj', '.gitignore')]: 'node_modules\n' });
  upsertMaster({ scope: 'project', projectPath: PROJ, homeDir: HOME, id: 'notion', entry: NOTION, io });
  assert.match(io.files[abs('proj', '.gitignore')], /connections\.json/);
  upsertMaster({ scope: 'project', projectPath: PROJ, homeDir: HOME, id: 'linear', entry: LINEAR, io });
  const hits = io.files[abs('proj', '.gitignore')].split('\n').filter((l) => l === 'connections.json');
...
test('upsertMaster outside a git repo leaves .gitignore alone', () => {
  const io = memIo();
  upsertMaster({ scope: 'project', projectPath: PROJ, homeDir: HOME, id: 'notion', entry: NOTION, io });
  assert.ok(!(abs('proj', '.gitignore') in io.files));
});
...
test('writeCodexBlock appends once, then replaces only between markers', () => {
  const io = memIo({ [abs('proj', '.codex', 'config.toml')]: '# theirs\nmodel = "gpt-5"\n' });
  writeCodexBlock({ file: abs('proj', '.codex', 'config.toml'), masters: { notion: NOTION }, io });
  const first = io.files[abs('proj', '.codex', 'config.toml')];
  assert.match(first, /^# theirs\nmodel = "gpt-5"\n/);
  assert.match(first, /# nami:connections start/);
  writeCodexBlock({ file: abs('proj', '.codex', 'config.toml'), masters: { linear: LINEAR }, io });
  const second = io.files[abs('proj', '.codex', 'config.toml')];
...
test('writeCodexBlock creates the file when missing and refuses on two start markers', () => {
  const io = memIo();
  writeCodexBlock({ file: abs('x', 'config.toml'), masters: { notion: NOTION }, io });
  assert.match(io.files[abs('x', 'config.toml')], /mcp_servers\.notion/);
  const twice = '# nami:connections start\n# nami:connections end\n# nami:connections start\n# nami:connections end\n';
  const io2 = memIo({ [abs('x', 'config.toml')]: twice });
  const res = writeCodexBlock({ file: abs('x', 'config.toml'), masters: {}, io: io2 });
  assert.equal(res.ok, false);
  assert.equal(io2.files[abs('x', 'config.toml')], twice);
});

test('writeCodexBlock skips ids already defined outside the block (no duplicate tables)', () => {
  const io = memIo({ [abs('x', 'config.toml')]: '[mcp_servers.notion]\ncommand = "theirs"\n' });
  writeCodexBlock({ file: abs('x', 'config.toml'), masters: { notion: NOTION, linear: LINEAR }, io });
  const out = io.files[abs('x', 'config.toml')];
...
test('deliveryPlan project scope: json merges for the four natives + opencode translation + codex block', () => {
  ...
  assert.equal(byAgent['claude:json'].file, abs('proj', '.mcp.json'));
  assert.equal(byAgent['claude:json'].section, 'mcpServers');
  assert.equal(byAgent['cursor:json'].file, abs('proj', '.cursor', 'mcp.json'));
  assert.equal(byAgent['gemini:json'].file, abs('proj', '.gemini', 'settings.json'));
  assert.equal(byAgent['kimi:json'].file, abs('proj', '.kimi-code', 'mcp.json'));
  assert.equal(byAgent['opencode:json'].file, abs('proj', 'opencode.json'));
  assert.equal(byAgent['opencode:json'].section, 'mcp');
  assert.equal(byAgent['opencode:json'].entries.notion.type, 'local');
  assert.equal(byAgent['codex:block'].file, abs('proj', '.codex', 'config.toml'));
  assert.equal(byAgent['grok:block'].file, abs('proj', '.grok', 'config.toml'));
...
test('deliveryPlan user scope: claude goes via its own CLI, codex block lands in ~/.codex', () => {
  ...
  assert.equal(codex.file, abs('home', 'u', '.codex', 'config.toml'));
  const cursor = plan.find((s) => s.agent === 'cursor');
  assert.equal(cursor.file, abs('home', 'u', '.cursor', 'mcp.json'));
});
...
test('deliveryPlan user scope: grok’s notebook is ~/.grok/config.toml', () => {
  ...
  assert.equal(plan[0].file, abs('home', 'u', '.grok', 'config.toml'));
});
...
test('readNotebooks + coverage: who has it, who is missing it', () => {
  const io = memIo({
    [abs('proj', '.mcp.json')]: JSON.stringify({ mcpServers: { notion: NOTION } }),
    [abs('home', 'u', '.cursor', 'mcp.json')]: JSON.stringify({ mcpServers: { notion: NOTION, n8n: { command: 'x' } } }),
    [abs('home', 'u', '.codex', 'config.toml')]: '[mcp_servers.notion]\ncommand = "npx"\n',
    [abs('home', 'u', '.grok', 'config.toml')]: '[cli]\ninstaller = "internal"\n\n[mcp_servers.notion]\ncommand = "npx"\n',
    [abs('home', 'u', '.hermes', 'config.yaml')]: 'mcp_servers:\n  other:\n    command: y\n',
  });
...
test('runPlan merges json, writes the block, records cli and manual honestly', async () => {
  ...
  const io = memIo({ [abs('proj', 'opencode.json')]: JSON.stringify({ mcp: { theirs: { type: 'local' } } }) });
  const ran = [];
  const plan = deliveryPlan({
    masters: { notion: NOTION }, scope: 'project',
  ...
  assert.deepEqual(JSON.parse(io.files[abs('proj', '.mcp.json')]).mcpServers.notion, NOTION);
  const oc = JSON.parse(io.files[abs('proj', 'opencode.json')]);
  assert.ok(oc.mcp.theirs, 'hand-made opencode entry preserved');
  assert.equal(oc.mcp.notion.type, 'local');
  assert.match(io.files[abs('proj', '.codex', 'config.toml')], /mcp_servers\.notion/);
  assert.match(io.files[abs('proj', '.grok', 'config.toml')], /mcp_servers\.notion/);
...
test('antigravity aliases the gemini notebook, and unknown tools never read as missing', () => {
  ...
  assert.equal(plan[0].file, abs('proj', '.gemini', 'settings.json'));
  const io = memIo({ [abs('proj', '.mcp.json')]: JSON.stringify({ mcpServers: { notion: NOTION } }) });
...
test('nami-browser is reserved and never written to the master', () => {
  ...
  assert.equal(res.ok, false);
  assert.ok(!(abs('proj', 'connections.json') in io.files), 'file must not be created');
});

test('readMaster drops a hand-pasted nami-browser so delivery cannot copy it', () => {
  const io = memIo({
    [abs('proj', 'connections.json')]: JSON.stringify({
      mcpServers: {
        notion: NOTION,
...
test('runPlan does not write a nami-browser json entry', async () => {
  ...
  const results = await runPlan({
    plan: [{
      agent: 'claude', kind: 'json', file: abs('proj', '.mcp.json'), section: 'mcpServers',
      entries: { notion: NOTION, 'nami-browser': { type: 'http', url: 'http://127.0.0.1:9/secret' } },
    }],
    io, execCmd: async () => ({ ok: true }),
  });
  assert.equal(results[0].ok, true);
  const doc = JSON.parse(io.files[abs('proj', '.mcp.json')]);
