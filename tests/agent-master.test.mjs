...
function memIo(seed = {}) {
  ...
  return {
    ...
    // sep, not '/': the module under test joins with path.join, so on Windows
    // every key in here is a backslash path and a '/' prefix test matched
    // nothing at all — readAgentMasters reported an empty drawer.
    list: (dir) => Object.keys(files).filter((f) => f.startsWith(dir + sep)).map((f) => f.slice(dir.length + 1)).filter((r) => !r.includes(sep)),
    files,
  };
}
...
test('readAgentMasters lists agents/ and parses each', () => {
  const io = memIo({ [abs('proj', 'agents', 'release-scribe.md')]: MASTER_MD, [abs('proj', 'agents', 'notes.txt')]: 'x' });
...
test('agentDeliveryPlan: one copy per writing tool, cursor rides claude, hermes named', () => {
  ...
  assert.equal(by.claude.file, abs('proj', '.claude', 'agents', 'release-scribe.md'));
  assert.equal(by.opencode.file, abs('proj', '.opencode', 'agents', 'release-scribe.md'));
  assert.equal(by.gemini.file, abs('proj', '.gemini', 'agents', 'release-scribe.md'));
  assert.equal(by.kimi.file, abs('proj', '.kimi-code', 'agents', 'release-scribe.md'));
  assert.equal(by.codex.file, abs('proj', '.codex', 'agents', 'release-scribe.toml'));
...
test('deliverAgents writes marked copies but never overwrites a hand-made file', () => {
  const io = memIo({
    [abs('proj', 'agents', 'release-scribe.md')]: MASTER_MD,
    [abs('proj', '.claude', 'agents', 'release-scribe.md')]: '---\nname: release-scribe\n---\ntheir own version\n',
  });
  const results = deliverAgents({ projectPath: PROJ, agentIds: ['claude', 'opencode', 'codex'], io });
  assert.match(io.files[abs('proj', '.opencode', 'agents', 'release-scribe.md')], /mode: all/);
  assert.match(io.files[abs('proj', '.codex', 'agents', 'release-scribe.toml')], /developer_instructions/);
  assert.match(io.files[abs('proj', '.claude', 'agents', 'release-scribe.md')], /their own version/, 'hand-made wins');
...
test('deliverAgents regenerates a previously marked copy', () => {
  const io = memIo({ [abs('proj', 'agents', 'release-scribe.md')]: MASTER_MD });
  deliverAgents({ projectPath: PROJ, agentIds: ['claude'], io });
  io.files[abs('proj', 'agents', 'release-scribe.md')] = MASTER_MD.replace('Keep the README honest.', 'Keep the CHANGELOG honest.');
  deliverAgents({ projectPath: PROJ, agentIds: ['claude'], io });
  assert.match(io.files[abs('proj', '.claude', 'agents', 'release-scribe.md')], /CHANGELOG honest/);
});
...
test('liftToMaster raises a claude agent to the drawer and marks the original as a copy', () => {
  ...
  const io = memIo({ [abs('proj', '.claude', 'agents', 'code-reviewer.md')]: theirs });
  const res = liftToMaster({ filePath: abs('proj', '.claude', 'agents', 'code-reviewer.md'), platform: 'claude', projectPath: PROJ, io });
  assert.equal(res.ok, true);
  assert.equal(res.masterPath, abs('proj', 'agents', 'code-reviewer.md'));
  assert.match(io.files[abs('proj', 'agents', 'code-reviewer.md')], /description: Reviews diffs\./);
  assert.ok(!isDelivered(io.files[abs('proj', 'agents', 'code-reviewer.md')]), 'the master is nobody\'s copy');
  assert.ok(isDelivered(io.files[abs('proj', '.claude', 'agents', 'code-reviewer.md')]), 'original is now a marked copy');
});

test('liftToMaster maps opencode mode into the superset and refuses a name clash', () => {
  const io = memIo({
    [abs('proj', '.opencode', 'agents', 'tester.md')]: '---\ndescription: Runs tests.\nmode: subagent\n---\nRun them.\n',
    [abs('proj', 'agents', 'tester.md')]: MASTER_MD,
  });
  const res = liftToMaster({ filePath: abs('proj', '.opencode', 'agents', 'tester.md'), platform: 'opencode', projectPath: PROJ, io });
  assert.equal(res.ok, false, 'a master already owns this name');
  const io2 = memIo({ [abs('proj', '.opencode', 'agents', 'tester.md')]: '---\ndescription: Runs tests.\nmode: subagent\n---\nRun them.\n' });
  const res2 = liftToMaster({ filePath: abs('proj', '.opencode', 'agents', 'tester.md'), platform: 'opencode', projectPath: PROJ, io: io2 });
  assert.equal(res2.ok, true);
  assert.match(io2.files[abs('proj', 'agents', 'tester.md')], /mode: subagent/);
});
...
test('sweepCopies removes marked copies only, and reports what it left', () => {
  ...
  io.files[abs('proj', '.opencode', 'agents', 'release-scribe.md')] = 'hand-made, same name\n';
  const res = sweepCopies({ projectPath: PROJ, slug: 'release-scribe', io });
  assert.ok(!(abs('proj', '.claude', 'agents', 'release-scribe.md') in io.files));
  assert.ok(!(abs('proj', '.codex', 'agents', 'release-scribe.toml') in io.files));
  assert.ok(abs('proj', '.opencode', 'agents', 'release-scribe.md') in io.files, 'unmarked file survives');
  assert.deepEqual(res.left, [abs('proj', '.opencode', 'agents', 'release-scribe.md')]);
});

test('antigravity delivers to the user-scope gemini folder, the only one agy reads', () => {
  const plan = agentDeliveryPlan({ slug: 'x', agentIds: ['antigravity'], projectPath: PROJ, homeDir: abs('home', 'cal') });
  assert.equal(plan[0].file, abs('home', 'cal', '.gemini', 'agents', 'x.md'));
});
...
test('deliveryState reports here / soon / theirs / none / via', () => {
  const io = memIo({ [abs('proj', 'agents', 'release-scribe.md')]: MASTER_MD });
  deliverAgents({ projectPath: PROJ, agentIds: ['claude'], io });
  io.files[abs('home', 'cal', '.gemini', 'agents', 'release-scribe.md')] = 'hand-made, same name\n';
  const rows = deliveryState({
    projectPath: PROJ, slug: 'release-scribe',
    agentIds: ['claude', 'codex', 'antigravity', 'hermes', 'cursor'], io, homeDir: abs('home', 'cal'),
  });
...
test('liftToMaster refuses a TOML agent rather than mangling it', () => {
  const io = memIo({ [abs('proj', '.codex', 'agents', 'tomlish.toml')]: 'name = "tomlish"\ndescription = "d"\n' });
  const res = liftToMaster({ filePath: abs('proj', '.codex', 'agents', 'tomlish.toml'), platform: 'codex', projectPath: PROJ, io });
  assert.equal(res.ok, false);
  assert.match(res.error, /markdown/i);
  assert.equal(io.files[abs('proj', '.codex', 'agents', 'tomlish.toml')], 'name = "tomlish"\ndescription = "d"\n',
    'the user’s own file is untouched');
  assert.ok(!(abs('proj', 'agents', 'tomlish.md') in io.files), 'and no master was written');
});
...
test('importToMaster lifts a personal Codex TOML into agents/ and leaves the source alone', () => {
  const src = abs('home', 'u', '.codex', 'agents', 'legal-drafter.toml');
  ...
  assert.equal(res.ok, true);
  const master = io.files[abs('proj', 'agents', 'legal-drafter.md')];
...
test('importToMaster lifts a personal markdown agent the same way', () => {
  const src = abs('home', 'u', '.config', 'opencode', 'agent', 'sql-tuner.md');
  const md = '---\ndescription: Makes queries fast.\nmode: subagent\n---\n\nTune them.\n';
  const io = memIo({ [src]: md });
  const res = importToMaster({ filePath: src, projectPath: PROJ, io });
  assert.equal(res.ok, true);
  assert.match(io.files[abs('proj', 'agents', 'sql-tuner.md')], /description: Makes queries fast\./);
  assert.match(io.files[abs('proj', 'agents', 'sql-tuner.md')], /Tune them\./);
  assert.equal(io.files[src], md, 'the personal file is untouched');
});

test('importToMaster refuses a name a master already owns', () => {
  const io = memIo({
    [abs('proj', 'agents', 'legal-drafter.md')]: MASTER_MD,
    [abs('home', 'u', '.codex', 'agents', 'legal-drafter.toml')]: 'name = "legal-drafter"\ndescription = "d"\n',
  });
  const res = importToMaster({ filePath: abs('home', 'u', '.codex', 'agents', 'legal-drafter.toml'), projectPath: PROJ, io });
  assert.equal(res.ok, false);
  assert.match(res.error, /already exists/);
});

test('a TOML whose prompt discusses TOML does not leak keys into the master', () => {
  const src = abs('home', 'u', '.codex', 'agents', 'meta.toml');
  const io = memIo({ [src]:
    'name = "meta"\ndescription = "Talks about config."\n'
    + 'developer_instructions = """\nAgent files start with\nname = "not-me"\n"""\n' });
  const res = importToMaster({ filePath: src, projectPath: PROJ, io });
  assert.equal(res.ok, true);
  assert.match(io.files[abs('proj', 'agents', 'meta.md')], /^name: meta$/m);
  assert.match(io.files[abs('proj', 'agents', 'meta.md')], /not-me/, 'the prompt text itself survives into the body');
});

test('importToMaster reads the other TOML fence too', () => {
  const src = abs('home', 'u', '.codex', 'agents', 'quoter.toml');
  const io = memIo({ [src]: "name = \"quoter\"\ndescription = \"d\"\ndeveloper_instructions = '''\nname = \"inside\"\nBody here.\n'''\n" });
  const res = importToMaster({ filePath: src, projectPath: PROJ, io });
  assert.equal(res.ok, true);
  assert.match(io.files[abs('proj', 'agents', 'quoter.md')], /^name: quoter$/m, 'the fenced key stays in the body');
  assert.match(io.files[abs('proj', 'agents', 'quoter.md')], /Body here\./);
});

test('importToMaster refuses a missing project rather than resolving paths against nothing', () => {
  const res = importToMaster({ filePath: abs('home', 'u', '.codex', 'agents', 'x.toml'), projectPath: '', io: memIo() });
  assert.equal(res.ok, false);
});
...
test('a master reaches grok as project markdown', () => {
  const io = memIo({});
  io.write(abs('p', 'agents', 'scribe.md'), '---\nname: scribe\ndescription: writes notes\n---\n\nBody.\n');
  deliverAgents({ projectPath: abs('p'), agentIds: ['grok'], io });
  const copy = io.read(abs('p', '.grok', 'agents', 'scribe.md'));
...
test('deleting a master sweeps its grok copy too', () => {
  ...
  io.write(abs('p', 'agents', 'scribe.md'), '---\nname: scribe\n---\n\nBody.\n');
  deliverAgents({ projectPath: abs('p'), agentIds: ['grok'], io });
  assert.ok(io.exists(abs('p', '.grok', 'agents', 'scribe.md')));
  const { removed } = sweepCopies({ projectPath: abs('p'), slug: 'scribe', io });
  assert.ok(removed.includes(abs('p', '.grok', 'agents', 'scribe.md')));
});
